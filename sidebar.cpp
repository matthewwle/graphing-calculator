...
Sidebar::Sidebar(float left, float width):_left(left), _width(width){
    ...
    rect.setFillColor(sf::Color(255,255,255)); //(192,192,192)); //silver
...
void Sidebar::fillHistory(ifstream& input){
    string a = "";
    for (int i = 0; i < 30; i++){
        getline(input, a);
        items[i] = a;
...
void Sidebar::writeHistory(ofstream& output){
    string a = "";
    for (int i = 0; i < 30; i++){
        a = items[i];
        output << a << endl;
